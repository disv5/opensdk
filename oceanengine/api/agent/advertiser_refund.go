...
import (
	...
	"github.com/disv5/opensdk/oceanengine/model/agent"
)
...
func AdvertiserRefund(clt *core.SDKClient, accessToken string, req *agent.AdvertiserRefundRequest) (string, error) {
	var resp agent.AdvertiserRefundResponse
	err := clt.Post("2/agent/advertiser/refund/", req, &resp, accessToken)
	if err != nil {
		return "", err
	}
