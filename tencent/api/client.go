...
import (
	...
	apierrors "donson.com.cn/draining/internal/pkg/tencent/errors"
	"encoding/json"
	"encoding/xml"
	"errors"
	"fmt"
...
func (c *APIClient) prepareRequest(
	...
	if ctx != nil {
		...
		if tok, ok := ctx.Value(config2.ContextOAuth2).(oauth2.TokenSource); ok {
		...
		if auth, ok := ctx.Value(config2.ContextBasicAuth).(config2.BasicAuth); ok {
			localVarRequest.SetBasicAuth(auth.UserName, auth.Password)
		}

		// AccessToken Authentication
		if auth, ok := ctx.Value(config2.ContextAccessToken).(string); ok {
